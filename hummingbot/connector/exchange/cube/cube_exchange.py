...
import hummingbot.connector.exchange.cube.cube_utils as cube_utils
import hummingbot.connector.exchange.cube.cube_web_utils as web_utils
...
class CubeExchange(ExchangePyBase):
    ...
    @property
    def name(self) -> str:
        if self._domain == "cube_main":
            return "cube"
        else:
            return f"cube_{self._domain}"
    ...
    async def _place_order(self,
        ...
        type_str = self.cube_order_type(order_type)
    ...
    def _initialize_trading_pair_symbols_from_exchange_info(self, exchange_info: Dict[str, Any]):
        mapping = bidict()
        for symbol_data in filter(cube_utils.is_exchange_information_valid, exchange_info["result"]):
